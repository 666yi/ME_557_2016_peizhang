...
bool GLSpotLightSource::addVariablesToProgram(GLuint program, int variable_index)
{
	...
	_inner_cone_angleIdx = glGetUniformLocation(program, GetVariableName(_glsl_object, _glsl_names[1], variable_index).c_str());
	checkUniform(_inner_cone_angleIdx, GetVariableName(_glsl_object, _glsl_names[1], variable_index));

	_outer_cone_angleIdx = glGetUniformLocation(program, GetVariableName(_glsl_object, _glsl_names[2], variable_index).c_str());
	checkUniform(_outer_cone_angleIdx, GetVariableName(_glsl_object, _glsl_names[2], variable_index));

	_cone_directionIdx = glGetUniformLocation(program, GetVariableName(_glsl_object, _glsl_names[3], variable_index).c_str());
	checkUniform(_cone_directionIdx, GetVariableName(_glsl_object, _glsl_names[3], variable_index));
...
void GLAppearance::addLightSource(GLLightSource& light_source)
{
	...
	if (_finalized)
	{
		cerr << "Appearance already finalized. Light cannot be added at this point." << endl;
		return;
	}
...
void GLAppearance::setMaterial(GLMaterial& material)
{
	...
	if (_finalized)
	{
		cerr << "Appearance already finalized. Material cannot be set" << endl;
		return;
	}
