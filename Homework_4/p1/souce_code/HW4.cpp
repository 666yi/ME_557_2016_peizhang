...
-----------------------------------------------------------------------------------*/
	...
	glGenVertexArrays(2, &vaoID[0]); // Create our Vertex Array Object-------------------------------------(Pei: Why we need 2??????)
	...
	glVertexAttribPointer(texAttrib, 2, GL_FLOAT, GL_TRUE, 5 * sizeof(GLfloat),(const GLvoid*)(3 * sizeof(GLfloat)));

//	glVertexAttribPointer((GLuint)0, 3, GL_FLOAT, GL_FALSE, 0, 0); // Set up our vertex attributes pointer
//	glEnableVertexAttribArray(0); // Disable our Vertex Array Object--------------------------------------------------(Pei: This statement maybe wrong according to function reference manual)
	...
	glVertexAttribPointer(colAttrib, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(GLfloat), 0); // Set up our vertex attributes pointer
...
//	glActiveTexture(GL_TEXTURE1);
	...
	std::string filename = "./tiger.bmp";
	unsigned char *g_image_data = ImgLoader::Load(filename);
	...
	glUniform1i(glGetUniformLocation(program, "tiger"), 0);
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 344, 343, 0, GL_BGRA, GL_UNSIGNED_BYTE, g_image_data);
