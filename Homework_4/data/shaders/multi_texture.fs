...
void main(void)                                                   
{
    ...
    vec4 tex_color_light =  texture(texture_midground, pass_TexCoord);
    
		vec4 tex_color_light_light =  texture(texture_foreground, pass_TexCoord);
    ...
    if(texture_blend == 0)
    {
        color = 0.1 * pass_Color + 0.3*tex_color + 0.3*tex_color_light+ 0.3*tex_color_light_light ;
    }
    else if(texture_blend == 1)
    {
        color = tex_color * tex_color_light*tex_color_light_light;
    }
