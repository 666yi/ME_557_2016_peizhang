...
vec4 useLight(Light light, vec4 surfacePostion, vec4 normal_transformed, vec3 normal, Material material )
{
    ...
    // Diffuse color
		vec3 diffuse_after=vec3(min(material.diffuse.x,light.light_color.x),min(material.diffuse.y,light.light_color.y),min(material.diffuse.z,light.light_color.z));
    float diffuse_coefficient = max( dot(normal_transformed, surface_to_light), 0.0);
    vec3 out_diffuse_color = diffuse_after  * diffuse_coefficient * light.diffuse_intensity;
    
    
    // Ambient color
		vec3 ambient_after=vec3(min(material.ambient.x,light.light_color.x),min(material.ambient.y,light.light_color.y),min(material.ambient.z,light.light_color.z));
		vec3 out_ambient_color = ambient_after * light.ambient_intensity;
    ...
    if(light.light_position.w == 1.0)
    {
        ...
        if(light_to_surface_angle > light.outer_cone_angle){
            attenuation = 0.0;
        }
    
				if(light_to_surface_angle <= light.outer_cone_angle && light_to_surface_angle >= light.inner_cone_angle){
					smooth_coefficient=1.0-smoothstep(light.inner_cone_angle,light.outer_cone_angle,light_to_surface_angle);
				}
    }
    ...
    vec3 linearColor = out_ambient_color  + smooth_coefficient*attenuation * ( out_diffuse_color + out_specular_color);
